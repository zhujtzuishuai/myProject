...
const { ccclass, executeInEditMode, disallowMultiple } = _decorator;
 ...
 */
@ccclass('AutoFitContainer')
@executeInEditMode
@disallowMultiple
export class AutoFitContainer extends Component {
    ...
    private _checkConflicts(): boolean {
        ...
        if (hasLayout || hasWidget) {
            if (!this._conflictLogged) {
                ...
                if (hasLayout) {
                    console.error(
                        `[AutoFitContainer] 节点 "${this.node.name}" 同时挂载了 Layout 组件，` +
                ...
                if (hasWidget) {
                    console.error(
                        `[AutoFitContainer] 节点 "${this.node.name}" 同时挂载了 Widget 组件，` +
    ...
    private _doFit(): void {
        ...
        const shiftY = minY + ay * newH;

        // 幂等守卫：状态已正确则跳过，避免浮点累积误差
        const EPS = 0.5;
        const oldSize    = myUit.contentSize;
        const needResize = Math.abs(oldSize.width  - newW) > EPS
                        || Math.abs(oldSize.height - newH) > EPS;
