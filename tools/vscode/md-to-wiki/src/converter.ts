...
export function mdToWiki(md: string): string {
...
function inlineToWiki(text: string): string {
 ...
 */
function safeInlineCode(inner: string): string {
    if (/[+\-*{}]/.test(inner)) {
        return `[${inner}]`;
    }
    return `{{${inner}}}`;
...
function escapeInsideCode(line: string): string {
    return line;
}
